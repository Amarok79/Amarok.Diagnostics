...
{
    /// <summary>
    ///     Serializes and enqueues a single <see cref="Activity"/> instance. <see cref="FlushAsync"/> must
    ...
    ///     enqueued by multiple calls to <see cref="Write"/> can be flushed to persistence storage by
    ///     calling <see cref="FlushAsync"/> once afterward.
    /// </summary>
    ValueTask FlushAsync();

    /// <summary>
    ///     Exports all trace log files into a Zip file at the given location. Calls to <see cref="Write"/>
    ///     and <see cref="FlushAsync"/> must complete before starting an export. The method returns after
    ///     starting the export operation and returns a second task which is completed at the end of the
    ///     export operation.
    /// </summary>
